...
-- Concentrado con estatus del expediente
...
-- Concentrado final con gerencias
...
SELECT 
    ...
    t1.CDRAUTOSURTIDO,
    CASE
        WHEN CAST(t2.IDREGIONGEOGRAFICA AS INT64) IN (1,2,9,11) THEN 'NORTE'
        WHEN CAST(t2.IDREGIONGEOGRAFICA AS INT64) IN (4,5,7,10) THEN 'CENTRO'
        WHEN CAST(t2.IDREGIONGEOGRAFICA AS INT64) IN (3,6,8,12) THEN 'OCCIDENTE'
        ELSE 'SIN GERENCIA'
...
SELECT 
    ...
    t2.AUTORIZADOR
FROM `STG_PREVENCION_FRAUDES.QUERY_FOR_ENVIOHISTORICO` t1
INNER JOIN `STG_PREVENCION_FRAUDES.DE_VISTA_VALE` t2 ON t1.IDEXPEDIENTE = t2.IDEXPEDIENTE
...
LEFT JOIN `STG_PREVENCION_FRAUDES.DE_VALEHISTORICO` t2 ON t1.IDVALE = t2.IDVALE
WHERE t2.IDCOSTO IS NOT NULL AND t1.FECHAEXPEDICION >= '2023-01-01 00:00:00';

-- Agregar causa de cambio de vale
...
SELECT 
    ...
    t1.TIPOASIGNACION1,
    t1.AUTOMATICO,
    t1.MANUAL_CON_ALERTA,
    t1.MANUAL_COMPRAS,
    t1.MANUAL_SEGUIMIENTO,
    t1.DIRECTA,
    t1.AREAASIGNACION,
    t1.PIEZAAUTOSURTIDO,
    t1.MONTOCONVENIO
...
INNER JOIN `STG_PREVENCION_FRAUDES.DE_COSTO` t2 ON t1.IDCOSTO = t2.IDCOSTO AND t1.TIPO = 'COSTO';
...
-- ================================================================================
-- FASE 9: TABLA FINAL (SIN HOMOLOGACIÓN DE USUARIOS)
-- ================================================================================

-- NOTA: La tabla USUARIOHOMOLOGADO no está disponible aún
-- Por ahora creamos la tabla final sin el campo USUARIOHOMOLOGADO
...
SELECT 
    ...
    -- USUARIOHOMOLOGADO - Campo pendiente por tabla faltante
    CASE 
...
-- ================================================================================
-- QUERY PARA ACTUALIZAR CON USUARIOHOMOLOGADO (EJECUTAR CUANDO ESTÉ DISPONIBLE)
-- ================================================================================

/*
-- Una vez que se tenga la tabla USUARIOHOMOLOGADO, ejecutar este UPDATE:

UPDATE `DM_PREVENCION_FRAUDES.TODASLASPIEZAS` t1
SET USUARIOHOMOLOGADO = (
    SELECT 
        CASE 
            ...
            ELSE " "
        END
    FROM `LAN_VERIFICACIONES.USUARIOHOMOLOGADO` t2 
    WHERE t1.USUARIO = t2.UsuarioBase
    LIMIT 1
)
WHERE TRUE;

-- O crear nuevamente la tabla con el LEFT JOIN:
...
SELECT 
    ...
    -- USUARIOHOMOLOGADO con tabla disponible
    CASE 
        WHEN t1.USUARIO = t2.usuariobase THEN t2.usuariohomologado
        WHEN t1.USUARIO IS NULL OR t1.USUARIO LIKE '%Valuador%' THEN SUBSTR(t1.USUARIO, 10, 5)
        WHEN t1.USUARIO = 'AUT' THEN "AUTOMATICO"
    ...
    t1.MONTOCONVENIO
FROM `STG_PREVENCION_FRAUDES.QUERY_FOR_TLP` t1
LEFT JOIN `LAN_VERIFICACIONES.USUARIOHOMOLOGADO` t2 ON t1.USUARIO = t2.UsuarioBase
WHERE t1.FECVALUACION >= '2024-01-01 00:00:00';
*/
...
1. Ejecutar todas las tablas de staging (STG_PREVENCION_FRAUDES.*)
2. Ejecutar la tabla final temporal (DM_PREVENCION_FRAUDES.TODASLASPIEZAS)
3. Una vez disponible USUARIOHOMOLOGADO, ejecutar el UPDATE o recrear la tabla final

TABLAS CREADAS:
- 31 tablas intermedias en STG_PREVENCION_FRAUDES
- 1 tabla final en DM_PREVENCION_FRAUDES.TODASLASPIEZAS

FILTROS APLICADOS:
- Expedientes con autorización >= 2019-01-01
- Vales con expedición >= 2023-01-01  
- Valuaciones >= 2024-01-01
- Solo refacciones (CONCEPTO = 'REF')
- Solo registros con IDCOSTO válido

CAMPOS FINALES: 87 campos con información completa del ciclo de vida de piezas

FASES COMPLETADAS:
✅ FASE 1: Extracción de catálogos (25 tablas)
✅ FASE 2: Procesamiento de fechas de autorización (5 tablas)
✅ FASE 3: Consolidación de información de prestadores (2 tablas)
✅ FASE 4: Preparación de información base (5 tablas)
✅ FASE 5: Primer concentrado (4 tablas)
✅ FASE 6: Procesamiento de vales y piezas (8 tablas)
✅ FASE 7: Cálculo de tiempos de entrega (4 tablas)
✅ FASE 8: Clasificación de asignaciones y columnas calculadas (1 tabla)
✅ FASE 9: Tabla final con USUARIOHOMOLOGADO temporal (1 tabla)

TOTAL: 55 queries organizadas en 9 fases completas

CONSIDERACIONES ESPECIALES:
- Campo USUARIOHOMOLOGADO implementado temporalmente con lógica del código SAS original
- Una vez disponible tabla USUARIOHOMOLOGADO, usar queries comentadas para actualización
- Todas las funciones SAS convertidas a equivalentes BigQuery
- Preservada toda la lógica de negocio original
*/