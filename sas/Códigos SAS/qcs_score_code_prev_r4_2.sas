...
* TITULO: Creación de Score para la Regla 4.2                                                     *
* OBJETIVO: Determinar Score para la familia 4 en la regla 4.2                                    *
* ENTRADAS: QCS_PREP_PREV_R4_2_ENGINE                                                             *
* SALIDAS: QCS_PREP_PREV_R4_2_SCORE                                                               *
***************************************************************************************************;
*------------------------*
...
%if not(%sysfunc(exist(&LIBN_SAS..QCS_PREP_PREV_R4_2_SCORE))) %then %do;
Proc sql;
create table &LIBN_SAS..QCS_PREP_PREV_R4_2_SCORE  (
...
PROC SQL;
	CREATE TABLE WORK.BENEF_ALERTADO AS
		...
		FROM
			&LIBN_SAS..QCS_PREP_PREV_R4_2_ENGINE t1
			where  flg_alert_pago=1 and datepart(PERIOD_BATCHDATE) = &fec_fin.;	
	;
QUIT;
RUN;
*-------------------------------------------*
...
PROC SQL;
	CREATE TABLE WORK.RANK_SUM AS
		SELECT
			t1.*,
			SUM(0.7 * t1.RANK1_SUM_PAG_N, 0.3 * t1.RANK2_SUM_PAG_SUM) AS RANK_SUM
		FROM
			WORK.RANKS t1
	;
QUIT;
...
*------------------------------------*;
PROC SQL;
	CREATE TABLE WORK.QCS_PREP_PREV_R4_2_SCORE AS
		SELECT 
			TIPO_PERSONA,
            BENEFICIARIO,
			SUM_PAG_N,
...
RUN;
*---------------------------------------------*
* Genera la tabla del score para la Regla 4.2 *
*---------------------------------------------*;
PROC SQL;
connect to oracle
(user=&Res_ora_user. password=&Res_ora_password. path=&Res_ora_path.);
execute (delete from  QCS_PREP_PREV_R4_2_SCORE
...
PROC SQL;
	INSERT INTO &LIBN_SAS..QCS_PREP_PREV_R4_2_SCORE
		SELECT 
            TIPO_PERSONA,
            BENEFICIARIO,
			SUM_PAG_N,
