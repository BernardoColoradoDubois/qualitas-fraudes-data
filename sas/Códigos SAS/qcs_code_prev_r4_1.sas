...
* TITULO: Creaci�n de Prep-Table Familia 4 para la regla 4.1                                                                        *
* SALIDAS: QCS_PREP_PREV_R4_1_ENGINE  *
* Modificación de codigo en regla 4.1, solicitada por Equipo siniestros PDF el 05/05/2023. 
* Se agregan los campos "CONDUCTOR, FECHA REPORTE y REPORTE" al detalle de las prep-table, que se modifican son:
* •  QCS_PREP_PREV_R4_1_ENGINE
* •  stg_pagosproveedores
* •  TMP_QCS_PREP_PREV_R4_1_ENGINE
* Modificación realizada por Juan Erik P�rez (JEP) el 18/05/2023. 										 *
...
data LISTA_COD_TXT_incluye LISTA_BENEFICIARIOS_excluye;
	set &lib_resu..QCS_PARAM_PREV_R4_1;

	if upcase(PARAMETER_NAME)="VAR_NUMERO_PAGOS_X_BENEFICIARIO" then
		do;
			call symput("NUMERO_PAGOS_X_BENEFICIARIO", PARAMETER_VALUE);
		end;
	else if upcase(PARAMETER_NAME)="LISTA_COD_TXT" and missing(PARAMETER_VALUE) ne 
		1 then
			do;
			output LISTA_COD_TXT_incluye;
		end;
	else if upcase(PARAMETER_NAME)="LISTA_BENEFICIARIOS" and 
...
proc sql;
	create table stg_pagosproveedores as select a.FECHA_PAGO	, kstrip(a.COD_TXT) as 
        ...
        b.FEC_HORA_REP, /* Inclusion del campo FECHA REPORTE - JEP 18/05/2023, se eliminó el formato, ya que no era posible insertar los registros de forma correcta xm */
...
data poliza_flotilla;
	set stg_pagosproveedores (keep=POLIZA INCISO);
	length INCISO2 $10.;
...
proc sql;
	create table pagosproveedores_ind as select b.* from stg_pagosproveedores b 
		...
		datepart(FECHA_PAGO) between  &fec_inicio and &fec_fin and 
		kstrip(upcase(DOCUMENTO_PAGO)) ne 'PT' and UPCASE(KSTRIP(CODIGO_DE_PAGO))='I' 
		and kstrip(COD_TXT) in (select kstrip(PARAMETER_VALUE) from 
		LISTA_COD_TXT_incluye);
quit;
...
data lcasres.pagosproveedores_ind5;
	set lcasres.pagosproveedores_ind5;
	by ClusterId poliza SINIESTRO descending FECHA_PAGO;
	order_row=_N_;
run;

data lcasres.pagosproveedores_ind6;
	set lcasres.pagosproveedores_ind5;
	by ClusterId poliza SINIESTRO order_row;
...
proc sql;
	create table stg_produccion2 as select kstrip(POLIZA) as POLIZA_PROD, 
...
proc sort data=stg_produccion2;
...
data lcasres.TMP_QCS_PREP_PREV_R4_1_ENGINE
(keep=/*Datos pagosproveedores */
	FECHA_PAGO COD_TXT DOC_PAGO CVE_BENEF BENEFICIARIO OFICINA_ATENCION 
		...
		FLG_ALERT_PAGO CLUSTERID PERIOD_BATCHDATE BATCHDATE
...
data lcasres.proveedores_x_pagos;
	set lcasres.TMP_QCS_PREP_PREV_R4_1_ENGINE (keep=beneficiario std_beneficiario 
		flg_alert_pago);
	by std_beneficiario;
...
%delete_table_db_ora(tabnm=TMP_QCS_PREP_PREV_R4_1_ENGINE, lib=&lib_resu);

/* Save the CAS tables to the database */
proc casutil incaslib="CasUser" outcaslib="QCS_Resultados_Planeacion";
	save casdata="TMP_QCS_PREP_PREV_R4_1_ENGINE" 
		casout="TMP_QCS_PREP_PREV_R4_1_ENGINE" 
...
proc sql;
	connect to oracle (user=&user_db2
		orapw=&pwd_user_db2
		path="&path_db2");
	execute(delete from &schema_db2..QCS_PREP_PREV_R4_1_ENGINE where 
...
proc sql;
	connect to oracle (user=&user_db2
		orapw=&pwd_user_db2
		path="&path_db2");
	execute(insert into RESULTADOS.QCS_PREP_PREV_R4_1_ENGINE select FECHA_PAGO, 
		...
		TRIM(TIPO_PERSONA), PAGOS_X_BENEFICIARIO, FLG_ALERT_PAGO, CLUSTERID, 
		BATCHDATE, PERIOD_BATCHDATE from     
		&schema_db2.."TMP_QCS_PREP_PREV_R4_1_ENGINE") BY ORACLE;
	disconnect from oracle;
quit;
