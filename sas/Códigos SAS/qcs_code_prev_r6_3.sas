...
* TITULO: Creación de Prep-Table Familia 6 para la regla 6.3                                  *
* OBJETIVO: Determinar pagos de daños mayores a $7,500 autorizado por ajustadores             *
* ENTRADAS: PAGOSPROVEEDORES                                                                  *
* SALIDAS: QCS_PREP_PREV_R6_3_ENGINE, QCS_PREP_PREV_R6_3_DETAIL, QCS_PREP_PREV_R6_3_RESULT    *
***********************************************************************************************;
*------------------------*
...
PROC SQL;
    ...
    SELECT PARAMETER_VALUE FROM &LIBN_SAS..QCS_PARAM_PREV_R6_3
	WHERE NAME_OF_THE_RULE='PREVENCION 6.3' AND PARAMETER_NAME='LISTA_AJUSTADOR';
QUIT;
RUN;
...
PROC SQL;
    ...
    SELECT PARAMETER_VALUE FROM &LIBN_SAS..QCS_PARAM_PREV_R6_3
	WHERE NAME_OF_THE_RULE='PREVENCION 6.3' AND PARAMETER_NAME='LISTA_COD_TXT';
QUIT;
RUN;
*--------------------------------------------------------------*
...
PROC SQL;
    ...
    SELECT PARAMETER_VALUE FROM &LIBN_SAS..QCS_PARAM_PREV_R6_3
	WHERE NAME_OF_THE_RULE='PREVENCION 6.3' AND PARAMETER_NAME='LISTA_BENEFICIARIO';
QUIT;
RUN;
 
/*Busca el nombre de GR que le corresponde */ 
proc sql;
...
*-------------------------------------------*
* Obtiene el umbral para el Importe de Pago *
*-------------------------------------------*;
PROC SQL NOPRINT;
    SELECT PARAMETER_VALUE INTO :UMBRAL_IMPORTE_DE_PAGO FROM &LIBN_SAS..QCS_PARAM_PREV_R6_3
	WHERE NAME_OF_THE_RULE='PREVENCION 6.3' AND PARAMETER_NAME='UMBRAL_IMPORTE_DE_PAGO';
QUIT;
*---------------------------------------------------------------------*
...
connect to oracle as Insumos
(user=&Ins_ora_user. password=&Ins_ora_password. path=&Ins_ora_path.);
/*create table WORK.PAGOS_T1_ as */
create table WORK.tmp_PAGOSPROVEEDORES as
...
where  COD_TXT in (select distinct PARAMETER_VALUE from Work.LISTA_COD_TXT);
disconnect from Insumos;
QUIT;

data tmp_PAGOSPROVEEDORES;
   set tmp_PAGOSPROVEEDORES;
length tmp_CVE_NOMBRE_AJUSTADOR_GR $200.;
  var_unique = _N_;
   if  missing(CVE_AJU) = 0 and  missing(AJUSTADOR) = 0;
run;

/*Asigna el valor de GR de agente a la fuente de pagosproveedores */
proc sql;
 create table  tmp_PAGOSPROVEEDORES2 as 
select  
        p.var_unique,
        p.FECHA_PAGO, 
        p.COD_TXT, 
  ...
  /*Agrega el GR de Agente */
   case
       ...
       end  as  CVE_NOMBRE_AJUSTADOR_GR length=360,
...
proc sort data=tmp_PAGOSPROVEEDORES2 nodupkey;
  by var_unique;
quit;

/* Filtra por lista de exclución*/
proc sql;
 create table  PAGOS_T1 as 
select 
        p.FECHA_PAGO, 
        p.COD_TXT, 
		p.COBERTURA,
        ...
        p.BENEFICIARIO_GR
 from  tmp_PAGOSPROVEEDORES2 p
 where  BENEFICIARIO_GR not in (select distinct  BENEFICIARIO_GR from LISTA_BENEFICIARIO2 )
 and  CVE_NOMBRE_AJUSTADOR_GR not in (select distinct CVE_NOMBRE_AJUSTADOR_GR from LISTA_AJUSTADOR2);
quit;

*------------------------------------------------------------------------------------------------------------------*
* Realiza la suma del importe de pago y obtiene el numero de pagos por ajustador, poliza, siniestro y beneficiario *
*------------------------------------------------------------------------------------------------------------------*;
proc sort data=PAGOS_T1;
   by  CVE_NOMBRE_AJUSTADOR_GR POLIZA NRO_SINIESTRO BENEFICIARIO_GR;
quit;

DATA WORK.PAGOS_T2  (RENAME=(SUM_IMPORTE_PAGO=IMPORTE_PAGO));
	SET WORK.PAGOS_T1;
	BY CVE_NOMBRE_AJUSTADOR_GR POLIZA NRO_SINIESTRO BENEFICIARIO_GR;
	RETAIN NUM_PAGOS SUM_IMPORTE_PAGO;

	IF First.BENEFICIARIO_GR THEN
		DO;
			SUM_IMPORTE_PAGO=0;
			NUM_PAGOS=0;
		END;
	    ...
	    SUM_IMPORTE_PAGO = sum (SUM_IMPORTE_PAGO, IMPORTE_PAGO);

   if last.BENEFICIARIO_GR;
     drop IMPORTE_PAGO;

RUN;

*----------------------------------------------------------------------*
* Identifica los ajustadores con pagos mayores a 7500 por beneficiario *
*----------------------------------------------------------------------*;
%put  &=UMBRAL_IMPORTE_DE_PAGO;
PROC SQL;
	CREATE TABLE WORK.PAGOS_T3 AS
		SELECT t1.*
		FROM WORK.PAGOS_T2 t1
	WHERE IMPORTE_PAGO &UMBRAL_IMPORTE_DE_PAGO.;
QUIT;
RUN;
*----------------------------------------------------*
* Genera las Prep Tables Motora, Detalle y Resultado *
*----------------------------------------------------*;
PROC SORT DATA=Work.PAGOS_T1 OUT=WORK.QCS_PREP_PREV_R6_3_ENGINE;
  BY AJUSTADOR BENEFICIARIO;
RUN;
PROC SQL;
connect to oracle
(user=&Res_ora_user. password=&Res_ora_password. path=&Res_ora_path.);
execute (truncate table QCS_PREP_PREV_R6_3_ENGINE) by oracle;
disconnect from oracle;
QUIT;
PROC SQL;
	INSERT INTO &LIBN_SAS..QCS_PREP_PREV_R6_3_ENGINE
    SELECT 
    ...
    FROM WORK.QCS_PREP_PREV_R6_3_ENGINE;
QUIT;
RUN;

PROC SORT DATA=Work.PAGOS_T3 OUT=WORK.QCS_PREP_PREV_R6_3_DETAIL;
  BY AJUSTADOR BENEFICIARIO;
RUN;
...
connect to oracle
(user=&Res_ora_user. password=&Res_ora_password. path=&Res_ora_path.);
execute (delete from QCS_PREP_PREV_R6_3_DETAIL 
...
PROC SQL;
	INSERT INTO &LIBN_SAS..QCS_PREP_PREV_R6_3_DETAIL
    SELECT 
       CVE_NOMBRE_AJUSTADOR_GR,
       NRO_SINIESTRO,
       COD_TXT,
       ...
       "&SYSUSERID." AS SYSUSERID,
       DHMS(&FEC_FIN.,0,0,0) AS PERIOD_BATCHDATE
    FROM WORK.QCS_PREP_PREV_R6_3_DETAIL;
QUIT;
RUN;
...
connect to oracle
(user=&Res_ora_user. password=&Res_ora_password. path=&Res_ora_path.);
execute (truncate table QCS_PREP_PREV_R6_3_RESULT) by oracle;
disconnect from oracle;
QUIT;
PROC SQL;
	INSERT INTO &LIBN_SAS..QCS_PREP_PREV_R6_3_RESULT
    SELECT 
       CVE_NOMBRE_AJUSTADOR_GR,
       FECHA_PAGO,
       AJUSTADOR ,
       ...
       BENEFICIARIO, 
	   NUM_PAGOS,
       IMPORTE_PAGO AS IMPORTE_PAGO_MAYOR_7M,
       DATETIME() as BATCHDATE,
       "&SYSUSERID." AS SYSUSERID,
       DHMS(&FEC_FIN.,0,0,0) AS PERIOD_BATCHDATE
    FROM WORK.QCS_PREP_PREV_R6_3_DETAIL;
