...
query = "SELECT * FROM `qualitasfraude.DM_FRAUDES.POLIZAS_VIGENTES` ORDER BY ID;"
...
cursor.execute('TRUNCATE TABLE INSUMOS.POLIZAS_VIGENTES')
sql="INSERT INTO INSUMOS.POLIZAS_VIGENTES"+" VALUES("+",".join([f":{i+1}" for i in range(df.shape[1])])+")"
