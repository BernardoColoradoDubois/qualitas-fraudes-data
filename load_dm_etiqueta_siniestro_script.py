...
query = "SELECT * FROM `qualitasfraude.DM_FRAUDES.DM_ETIQUETA_SINIESTRO` ORDER BY ID;"
...
cursor.execute('TRUNCATE TABLE INSUMOS.DM_ETIQUETA_SINIESTRO')
sql="INSERT INTO INSUMOS.DM_ETIQUETA_SINIESTRO"+" VALUES("+",".join([f":{i+1}" for i in range(df.shape[1])])+")"
