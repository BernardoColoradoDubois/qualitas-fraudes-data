...
SELECT 
  RECIBO AS ID_RECIBO
  ,CAST(CONCAT(SUBSTRING(FEC_VTO,7,4),'-',SUBSTRING(FEC_VTO,4,2),'-',SUBSTRING(FEC_VTO,1,2)) AS TIMESTAMP) AS FECHA_VENCIMIENTO
  ,POLIZA
  ,ENDOSO
  ,CONCAT(POLIZA,ENDOSO) AS POLIZA_ENDOSO
  ,SAFE_CAST(IMPORTE AS NUMERIC) AS IMPORTE
  ,REMESA
  ,CAST(CONCAT(SUBSTRING(MARCA_IMP,7,4),'-',SUBSTRING(MARCA_IMP,4,2),'-',SUBSTRING(MARCA_IMP,1,2)) AS TIMESTAMP) AS MARCA_IMP
  ,CAST(CONCAT(SUBSTRING(MARCA_ANUL,7,4),'-',SUBSTRING(MARCA_ANUL,4,2),'-',SUBSTRING(MARCA_ANUL,1,2)) AS TIMESTAMP) AS MARCA_ANUL
  ...
  ,CAST(CONCAT(SUBSTRING(FEC_APLICACION,7,4),'-',SUBSTRING(FEC_APLICACION,4,2),'-',SUBSTRING(FEC_APLICACION,1,2),' ',HORA_APLICA) AS TIMESTAMP) AS FECHA_APLICACION
  ,IMPRESION
