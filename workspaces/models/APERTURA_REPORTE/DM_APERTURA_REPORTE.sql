CREATE OR REPLACE TABLE `qlts-dev-mx-au-bro-verificacio.DM_CONTROL_FRAUDES.DM_APERTURA_REPORTE` AS
SELECT 
  ...
  SAFE_CAST(NULLIF(EMPLEADO, '') AS FLOAT64) AS EMPLEADO,
  ...
  -- Campos que pueden ser nulos
  SAFE_CAST(NULLIF(COD_AJUST, '') AS FLOAT64) AS COD_AJUST,
  SAFE_CAST(NULLIF(STATUS_INV_REP, '') AS FLOAT64) AS STATUS_INV_REP,
  SAFE_CAST(NULLIF(STATUS_INV_SIN, '') AS FLOAT64) AS STATUS_INV_SIN,
  
  -- Campos de texto
  ORGANISMO,
  SAFE_CAST(NULLIF(CVE_BENEF, '') AS FLOAT64) AS CVE_BENEF,
  BENEFICIO,
  DESCRIPCION,
  OF_AJUST,
  SAFE_CAST(NULLIF(PLACAS_CAB, '') AS FLOAT64) AS PLACAS_CAB,
  SAFE_CAST(NULLIF(DOBLE_DED, '') AS FLOAT64) AS DOBLE_DED,
  ABOGADO,
