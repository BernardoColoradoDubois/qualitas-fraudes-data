--CREATE OR REPLACE TABLE `STG_FRAUDES.STG_INCISOS_POLIZAS_1` AS
SELECT
  DI,
  SUBSTRING(DI,0,2) AS SECCION,
  SUBSTRING(DI,3,12) AS POLIZA,
  SUBSTRING(DI,13,18) AS ENDOSO,
  SUBSTRING(DI,19) AS ANEXO_INCISO,
  SUBSTRING(DI,3,18) AS POLIZA_ENDOSO,
  MARCA_COMP,
  MARCA_CORTA,
  AMIS,
  SUBRAMO,
  ...
  FECHA_CARGA,
FROM `sample_landing_sise.fraud_di`