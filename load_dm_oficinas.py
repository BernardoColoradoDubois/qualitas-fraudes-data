...
query = "SELECT * FROM `qualitasfraude.DM_FRAUDES.DM_OFICINAS` ORDER BY ID;"
...
cursor.execute('TRUNCATE TABLE INSUMOS.DM_OFICINAS')
sql="INSERT INTO INSUMOS.DM_OFICINAS"+" VALUES("+",".join([f":{i+1}" for i in range(df.shape[1])])+")"
